...
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
function [vertices,faces]=createsurface(bim)
id=1;
% Colors for plotting
red=[1.0 0.0 0.0];  
green=[0.5 1.0 0.5];  
...
%     id
    ...
    % Create surface from voxels
      if exist('bim','var')
          [faces,vertices]=voxel_bnd_faces(bim,sf,offset,id);
      end
    % Plot surface
    [fighndl]=plotsurf(faces,vertices,facecolor(id,:),edgecolor,name,fighndl);  
