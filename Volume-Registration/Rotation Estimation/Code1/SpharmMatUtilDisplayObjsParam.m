function SpharmMatUtilDisplayObjsParam(confs, objs, cpath)
...
for i = 1: 1  % numSbj
   % file = objs{i};
        ...
        case 'both'
            if overl == 1
                ...
                elseif subp == 2
                    ...
                    subplot(1,2,2);
                    patch_colormesh(sph_verts, faces, cdata); %title(sprintf('%s: ',titleStrRight));  %(sprintf('%s: ADC=%f',titleStrRight,adc));
                end
            else
                if subp == 0
    ...
    if ~isempty(suffix)
        suffix = sprintf('%s_%s',confs.Space(1:3),num2str(confs.count)); 
    else
       % suffix = confs.Space(1:3);
        suffix = num2str(confs.count);
    end
%     if ~isempty(rmsd)
...
%     
    ...
    switch strtrim(char(confs.Export))
        ...
        case 'both'
            ...
            saveas(h,fullfile(pa_png,[na '_' suffix]),'fig');
