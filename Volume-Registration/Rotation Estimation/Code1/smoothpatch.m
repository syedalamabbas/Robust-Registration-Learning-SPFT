function [FV2,vertices, faces]=smoothpatch(vertices, faces,mode,itt,lambda,sigma)
...
if(mode==1)
    ...
    Ne=vertex_neighbours(vertices, faces);
    % Do the curvature weighted smoothing
    [Vx,Vy,Vz]=smoothpatch_curvature_double(double(faces(:,1)),double(faces(:,2)),double(faces(:,3)),double(vertices(:,1)),double(vertices(:,2)),double(vertices(:,3)),ceil(double(itt)),[double(lambda) double(sigma)],Ne);
else
    % Do the umbrella inverse distance weighted smoothing
    [Vx,Vy,Vz]=smoothpatch_inversedistance_double(double(faces(:,1)),double(faces(:,2)),double(faces(:,3)),double(vertices(:,1)),double(vertices(:,2)),double(vertices(:,3)),ceil(double(itt)),[double(lambda) double(sigma)]);
end
