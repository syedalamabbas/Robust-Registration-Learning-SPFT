...
function [bim, new_name] = fix_holes(bim, confs)
...
for k=1:6
    ...
    roi = fill3dholes(roi,conn,epsilon,confs.name);
...
%disp(sprintf('fix_vox_num (%d) / vol (%d) = %f%%, save ...', ...
   ...
   new_name = [confs.OutDirectory '/' confs.name '.mat'];               
% Save surface object to a new file
%postfix = confs.name(1:end-4);
% if strcmp(postfix, 'bim') || strcmp(postfix,'fix')
%    % new_name = [confs.OutDirectory '/' name(1:end-3) num2str(confs.count) '_fix.mat'];
%     new_name = [confs.OutDirectory '/' confs.name(1:end-3) 'fix'];
% else
%    % new_name = [confs.OutDirectory '/' name num2str(confs.count) '_fix.mat'];
%      new_name = [confs.OutDirectory '/' confs.name  '_fix'];
% end   
