...
function SpharmMatUtilDisplayObjsNa(confs, objs, cpath)
...
for i = 1:numSbj
   ...
   %  [faces1,vertices1]=voxel_bnd_faces(bim,vxsize,origin,1);
    
%      vol2tri_fun(bim)

    end
          PlotSurface(vertices,faces,nastr,confs.theta); 
       
%     h = figure('Name', ['Object: ' nastr],'NumberTitle', 'off');
...
%             end
% 
%             saveas(h,fullfile(pa_png,[nastr '_' suffix '.png'])); close(h);
%         case 'both'
%             % do both
%             pa_png = fullfile(pa,'png');
%             if ~exist(pa_png,'dir')
...
%             end
% 
%             saveas(h,fullfile(pa_png,[nastr '_' suffix '.png']));            
%     end
%         
 end
